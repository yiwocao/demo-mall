...
class WritePost extends Component {
    ...
    getUserName(){
        return PostCtr.getUserName()
        .then((data) => {
                    this.setState({
                        updater:data.loginName,
                        blockData: data
                    }); 
                return data;
            });
    ...
    handleSubmit() {
        this.props.form.validateFields((error, param) => {
            if(!error){
                let {plateCode,storeCode,updater, files}  = this.state;
                if(!plateCode){
                    Toast.info("还未选中板块", 1);
                    return;
                }
                ...
                param.isPublish = 1;
                param.price1=Util.formatMoneyLarge(param.price1);
                let fileArr = [];
     ...
     */
    handleTextFocus() {
        this.setState({showEmoji: false,price1:Util.formatMoneyLarge(price1)});
    }
    handleTextFocus1(data) {
        this.setState({price1:Util.formatMoneyLarge(data.price1)});
    }
    /*
                ...
                <div className="write-post-form">
                    <List className="pop-over-write-comment-content write-post-list">
                        <InputItem {...getFieldProps('name', {
                            rules: [{required: true}],
                            initialValue: name
                        })} clear placeholder="名称" onFocus={this.handleTextFocus.bind(this)}/> {getFieldError('name')
                            ? <span class="comment-error-tip">名称不能为空</span>
                            : null}
                    </List>
                    <List className="pop-over-write-comment-content write-post-list">
                        <InputItem {...getFieldProps('slogan', {
                            rules: [{required: true}],
                            initialValue: slogan
                        })} clear placeholder="简介" onFocus={this.handleTextFocus.bind(this)}/> {getFieldError('slogan')
                            ? <span class="comment-error-tip">简介不能为空</span>
                            : null}
                    </List>
                    <List className="pop-over-write-comment-content write-post-list">
                        <InputItem {...getFieldProps('price1', {
                            rules: [{required: true}],
                            initialValue: price1
                        })} clear placeholder="价格" onFocus={this.handleTextFocus.bind(this)}/> {getFieldError('price1')
                            ? <span class="comment-error-tip">价格不能为空</span>
                            : null}
                    </List>
                    <List className="pop-over-write-comment-content write-post-no-bottom">
                        ...
                        /> {getFieldError('description')
                            ? <span class="comment-error-tip">商品详情不能为空</span>
                            : null}
                    </List>
