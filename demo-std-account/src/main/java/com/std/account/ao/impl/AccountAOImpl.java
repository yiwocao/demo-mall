...
import com.std.account.dto.res.XN002512Res;
...
public class AccountAOImpl implements IAccountAO {
	...
	@Override
	@Transactional
	public XN002512Res transAmountYu(String fromAccountNo, String toAccountNo, String channelType, String transAmount,
			String bizType, String bizNote ,String payGroup) {
		// TODO Auto-generated method stub
		XN002512Res res = new XN002512Res();
		Map<String, EChannelType> type = EChannelType.getChannelTypeResultMap();
        EChannelType echannelType = type.get(channelType);

        Account fromAccount = this.getAccount(fromAccountNo);
        Account toAccount = this.getAccount(toAccountNo);
        String systemCode = fromAccount.getSystemCode();
        
        Long fromAccountAmount = fromAccount.getAmount() - StringValidater.toLong(transAmount);
        Long toAccountAmount = toAccount.getAmount() + StringValidater.toLong(transAmount);

        // 特定账户余额可为负
        // !ESysAccount.getResultMap().containsKey(accountNumber)
        if (fromAccountAmount< 0 ) {
            throw new BizException("xn000000", "账户余额不足");
        }
        try {
			
        	// 记录流水
        	String lastOrder = jourBO.addChangedJour(systemCode, fromAccountNo,
        			echannelType, null, bizType, bizNote,
        			fromAccount.getAmount(), StringValidater.toLong(transAmount));
        	
        	
        	// 更改余额
        	fromAccount.setAmount(fromAccountAmount);;
        	fromAccount.setMd5(AccountUtil.md5(fromAccount.getMd5(), fromAccount.getAmount(),
        			fromAccountAmount));
        	fromAccount.setLastOrder(lastOrder);
        	
        	toAccount.setAmount(toAccountAmount);;
        	toAccount.setMd5(AccountUtil.md5(toAccount.getMd5(), toAccount.getAmount(),
        			toAccountAmount));
        	toAccount.setLastOrder(lastOrder);
        	
        	accountDAO.updateAmount(fromAccount);
        	accountDAO.updateAmount(toAccount);
        	res.setJourCode(lastOrder);
        	return res ;
		} catch (Exception e) {
			// TODO: handle exception
			 throw new BizException("xn000000", "账户流水信息修改失败");
		}
    }
