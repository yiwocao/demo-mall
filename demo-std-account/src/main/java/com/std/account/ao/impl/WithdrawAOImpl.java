...
import com.std.account.bo.impl.BankcardBOImpl;
...
public class WithdrawAOImpl implements IWithdrawAO {
    ...
    private void payOrderYES(Withdraw data, String payUser, String payNote,
        ...
        if (ECurrency.CNY.getCode().equals(account.getCurrency())
            ...
            accountBO.changeAmount(data.getAccountNumber(), EChannelType.Offline,
                null, null, data.getCode(), EJourBizType.AJ_QX, "线下取现支付",
                -data.getAmount());
        }
    }
	...
	@Override
	@Transactional
	public String applyOrder(String userId, Long amount, String applyNote,String systemCode) {
		User user = userBO.getRemoteUser(userId);
		if (amount <= 0) {
            throw new BizException("xn000000", "提现金额需大于零");
        }
		Account dbAccount = accountBO.getAccountByUser(userId, ECurrency.CNY.getCode());
		if(dbAccount == null){
			throw new BizException("xn000000", "未查询到对应的账户信息");
		}
		List<Bankcard> bankcardList = bankcardBO.queryBankcardList(userId, systemCode);
		String payCardInfo = null ;
		String payCardNo = null ;
		
		if(CollectionUtils.isNotEmpty(bankcardList)){
			Bankcard bankcard = bankcardList.get(0);
			payCardInfo = bankcard.getBankName()+bankcard.getSubbranch();
			payCardNo = bankcard.getBankcardNumber();
		}else{
			throw new BizException("xn000000", "该用户还未绑定银行卡");
		}
        // 判断本月是否次数已满，且现在只能有一笔取现未支付记录
        withdrawBO.doCheckTimes(dbAccount);
        if (dbAccount.getAmount() < amount) {
        ...
        amount = amount + fee;
        String withdrawCode = withdrawBO.applyOrder(dbAccount, amount, fee,
            payCardInfo, payCardNo, userId, applyNote);
        // 冻结取现金额
        accountBO.frozenAmount(dbAccount, amount, withdrawCode);
        return withdrawCode;
	}
