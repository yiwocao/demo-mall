...
export default class SmallImgBlock extends Component {
    ...
    render() {
        let {imgName, imgUrl, pathUrl,imgCode, index} = this.props;
        let imgBlock = "";
        ...
        if(result.outUrl){
            imgBlock = <div style={imgStyle} class="small-img-block"><Link to={`/user/shoplist?code=${imgCode}`}><div><img src={Util.formatImg(imgUrl)} alt={imgName}/></div><p>{imgName}</p></Link></div>
        }else{
            if(result.signin){
